...
structure Data :>
	...
	val new_data : Compiler.il_module option -> data
	    
	val set_il  : data * Compiler.il_module -> unit
	val set_nil : data * Compiler.nil_module -> unit
	val set_rtl : data * Compiler.rtl_module -> unit

	val clear : data -> unit

	val get_ilopt : data -> Compiler.il_module option
	val get_il    : data -> Compiler.il_module
	val get_nil   : data -> Compiler.nil_module
	val get_rtl   : data -> Compiler.rtl_module
    end =
struct
    val error = fn s => Util.error "Data" s
	
    datatype data' =
	ILOPT of Compiler.il_module option
      | IL of Compiler.il_module
      | NIL of Compiler.nil_module
      | RTL of Compiler.rtl_module
      | UNIT

    type data = data' ref
	
    fun new_data x = ref (ILOPT x)
	
    fun set_il (data, x)  = data := IL x
    fun set_nil (data, x) = data := NIL x
    fun set_rtl (data, x) = data := RTL x
	
    fun clear data = data := UNIT
	
    fun get_ilopt data = case !data of ILOPT x => x | _ => error "get_ilopt"
    fun get_il data    = case !data of IL x    => x | _ => error "get_il"
    fun get_nil data   = case !data of NIL x   => x | _ => error "get_nil"
    fun get_rtl data   = case !data of RTL x   => x | _ => error "get_rtl"
end
