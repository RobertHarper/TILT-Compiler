...
struct
    ...
    fun generate_il (notes as {unit, ...} : notes, data) =
	let val _ = Help.chat ("  [Compiling " ^ unit ^ " to assembly]\n")
	    val il_module = case Data.get_ilopt data
			      of NONE => elaborate (notes, true)
			       | SOME il_module => il_module
	in  Data.set_il (data, il_module)
	end
    
    fun generate_nil ({unit, ...} : notes, data) =
	let
	    val nil_module = if !Help.uptoElaborate then raise Stop
			     else Compiler.il_to_nil (unit, Data.get_il data)
	in  Data.set_nil (data, nil_module)
	end
    
    fun generate_rtl ({unit, ...} : notes, data) =
	let
	    val rtl_module = if (!Help.uptoPhasesplit) orelse (!Help.uptoClosureConvert) then raise Stop
			     else Compiler.nil_to_rtl (unit, Data.get_nil data)
	in  Data.set_rtl (data, rtl_module)
	end

    fun generate_asm ({asmFile, asmzFile, ...} : notes, data) =
	if (!Help.uptoRtl) then raise Stop
	else (remove asmzFile; (* In case we fail, avoid inconsistent copies *)
	      Compiler.rtl_to_asm (asmFile, Data.get_rtl data);
	      Data.clear data)

    fun generate (notes : notes, il_module') =
	let val arg = (notes, Data.new_data il_module')
	    val _ = app (fn f => f arg) [generate_il,
					 generate_nil,
					 generate_rtl,
					 generate_asm]
	in ()
...
(* First attempt (12:42 -- no change)
    fun generate_asm (notes as {asmFile, asmzFile, ...} : notes, rtl_module) =
	if (!Help.uptoRtl) then raise Stop
	else (remove asmzFile; (* In case we fail, avoid inconsistent copies *)
	      Compiler.rtl_to_asm (asmFile, rtl_module))

    fun generate_rtl (notes as {unit, ...} : notes, nil_module) =
	generate_asm (notes,
		      if (!Help.uptoPhasesplit) orelse (!Help.uptoClosureConvert) then raise Stop
		      else Compiler.nil_to_rtl (unit, nil_module))

    fun generate_nil (notes as {unit, ...} : notes, il_module) =
	generate_rtl (notes,
		      if !Help.uptoElaborate then raise Stop
		      else Compiler.il_to_nil (unit, il_module))

    fun generate_il (notes as {unit, ...} : notes, il_module') =
	generate_nil (notes,
		      let val _ = Help.chat ("  [Compiling " ^ unit ^ " to assembly]\n")
    ...
    fun generate arg = generate_il arg handle Stop => ()
*)
(* Original code (11:49)
