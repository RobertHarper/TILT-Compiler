(*
	...
	the possible status values are:
		...
		have up to date pinterface and tali files; PENDING
	...
	{I, A, B}. I may be compiled before A so that B may have an up
	to date pinterface file before A. Thus, it is possible for
...
XXX:
	...
	On the TAL backend, the manager can not assemble U before A's
	TAL interface (tali) is ready.  This seems to imply that U
	must wait for A to be compiled.

	We reduce the amount of parallelism for talx86 parallel makes;
	in this example, we have U wait for A to be compiled.

	Eventually, we should change the DAG.  For every node u that
	represents compiling a unit U with an ascribed interface I,
	add a second node ui that represents generating the tali file
	for U. In this example, the node for compiling U would depend
	on the node ai for generating A's tali file rather than the
	node a for compiling A.
...
struct
	...
	fun statusName (status:status) : string =
		(case status of
			WAITING => "waiting"
		|	READY _ => "ready"
		|	PENDING (_,interfaces) =>
	...
	local
		...
		fun get_targets (desc:I.desc, targets:targets) : Set.set =
			...
			in	(case (List.find notbound targets) of
					NONE =>
	...
	in
		...
		fun file_count () : int = Graph.nodecount (!graph)
	end
	...
	fun markReady (node : label) : unit =
		(case (get_status node) of
			WAITING => set_status (node, READY (Time.now()))
		|	READY _ => ()
	...
	*)
	...
	fun markPending' (node : label) : unit =
		let	fun newT (t,t') = (t, Time.-(Time.now(), t'))
			val status =
				(case (get_status node) of
					READY t => PENDING' (t, Time.zeroTime)
	...
	local
		...
		type slave_info =
			{status : slave_status ref,
			 in_channel : Comm.in_channel,
			 out_channel : Comm.out_channel}

		structure SlaveMap = SplayMapFn(
			type ord_key = Comm.identity
		...
		fun lookup_slave (slave : Comm.identity) : slave_info =
			(case (SlaveMap.find (!slaveMap, slave)) of
				NONE => error ("slave " ^ Comm.name slave ^ " missing")
			|	SOME info => info)
		...
		fun markSlaveIdle (slave : Comm.identity) : unit =
			let	val now = Time.now()
				val workingTime =
					(case (get_slave_status slave) of
						WORKING_SLAVE t => t
					|	IDLE_SLAVE _ =>
							error ("slave " ^ Comm.name slave ^
								" idle - making idle"))
			...
			let
				...
				fun addSlave slave =
					let	val _ = if slave_known slave then raise Stop
						...
						val slave_info =
							{status = ref (IDLE_SLAVE (Time.now())),
							 in_channel = in_channel,
							 out_channel = out_channel}
					in
						add_slave (slave, slave_info)
					end handle Stop => ()
	...
	in
		...
		fun waitForSlaves () : int * int =
			let
				val maxWorkingLocal = 1
				val newWorkingLocal = List.filter
					(fn (job, done) =>
				...
				val _ =
					app (fn slave =>
						(case (Comm.receive (get_in_channel slave)) of
							NONE => ()
						|	SOME Comm.READY => ()
...
|	_ =>
	...
	*)
	...
	fun stateSize ((a,b,c,d,e,f) : state) =
	...
	fun finalReport () : unit =
		let	val nodes = get_nodes()
			...
			val _ =
				(print (Int.toString (length underOne));
				 print " files under 1.0 second.\n")
			val _ =
				(print (Int.toString (length oneToTen));
				 print " files from 1.0 to 10.0 seconds.\n")
			val _ =
				(print (Int.toString (length tenToThirty));
				 print " files from 10.0 to 30.0 seconds:  ";
				 print_strings 40 (map #1 tenToThirty); print "\n")
			val _ =
				(print (Int.toString (length overThirty));
		...
		let
			fun useOne node =
				let	fun showSlave (name : string) : unit =
					...
					val _ =
						(case (get_status node) of
							PENDING (_,false) => markWorking node
					...
					if (!CheckpointVerbose) then
						...
						in
							String.concat [
								...
								Int.toString pending', " pending' + ",
								Int.toString working', " working' + ",
								Int.toString finished, " compiled = ",
								Int.toString total, " files"
							]
						end
	...
	fun make_lib (projects : string list, lib : string,
		...
		let	val _ = check_dir lib
			...
			fun mapper (pdec:I.pdec) : label option =
				(case pdec of
					I.UDEC {name=U,uexp=I.SRCU _,...} => SOME U
				|	_ => NONE)
