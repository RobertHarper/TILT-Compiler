...
struct
	...
	fun outputOf (command:string list) : string =
		let	val () =
				if !ShowTools then
					let	val msg = Listops.concatWith " " command
					in	print "running: "; print msg; print "\n"
					end
				else ()
		in	Util.outputOf command
		end
	...
	fun nproc () : int =
		let	val cmd = binFile "nproc"
			val nproc : string = outputOf [cmd]
			val nproc = getOpt(Int.fromString nproc, 1)
		in	nproc
		end
