...
{
  ptr_t *stack;        /* stack contains gray objects */
  long size;
  long cursor;
...
SharedStack_t *SharedStack_Alloc(int);
void SynchStart(SharedStack_t *);
void SynchMid(SharedStack_t *);
int SynchEnd(SharedStack_t *, Thunk_t *);  /* Returns 1 if gate was closed and global stack empty; i.e. global stack can not become non-empty */

int isEmptyGlobalStack(SharedStack_t *);
void moveToGlobalStack(SharedStack_t *, LocalStack_t *localStack);
void fetchFromGlobalStack(SharedStack_t *, LocalStack_t *localStack, int numToFetch);
