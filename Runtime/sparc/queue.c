...
void
SetInit(Set_t* set, long size)
{
	set->size = Max(16,size);
	set->data = (ptr_t *)emalloc(size*sizeof(ptr_t));
...
void
SetNormalizeExpand(volatile Set_t* s, int addSize)
{
	...
	|| 4 * effSize >= 3 * s->size) {
		newSize = 2 * effSize;
		newData = (ptr_t *) emalloc(newSize * sizeof(ptr_t));
	}
...
void
SetNormalize(volatile Set_t* s)
{
	SetNormalizeExpand(s, 0);
}

void
SetCopy(Set_t* from, Set_t* to)
{
	int srcLength = SetLength(from), targetLength = SetLength(to);
