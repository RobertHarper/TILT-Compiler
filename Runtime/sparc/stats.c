...
void reset_windowQuotient(WindowQuotient_t *wq, int fineness)
{
  ...
  wq->data = (char *) emalloc(sizeof(float) * wq->size);
  wq->first = 0;
  wq->last = 0;
  wq->onRemain = 0.0;
  wq->offRemain = 0.0;
...
{ 
  if (information >= 1) {
    ...
    printf("       NumGC         = %6d            NumMajorGC = %3d\n"
	   "       AvgFramsSize  = %4.0f         AvgStkDepth  = %4.0f           MaxStkDepth  = %4ld    newStkDepth = %4.0f\n",
	   NumGC, NumMajorGC, 
	   AvgStackFrameSize, AvgStackDepth, MaxStackDepth, AvgNewStackDepth);
    printf("       NumCopied     = %8ld     NumRoot      = %9ld      NumConflict  = %8ld\n"
	   "       NumShared     = %8ld     NumWrite     = %9ld      NumLocative  = %8ld\n",
    ...
    printf("\n");
    printf("MISC:  Total time    = %8.2f s   maxPhysMem   = %9ld      minPageFault = %8ld    invCtxtSwap  = %5ld\n"
	   "       Total Threads = %8d     sharedMem    = %9ld      majPageFault = %8ld    volCtxtSwap  = %5ld\n"
	   "       Processors    = %8d\n"
	   "       Max Threads   = %8d     unsharedData = %9ld                                 swapping     = %5ld\n"
	   "                                    unsharedStk  = %9ld\n",
    ...
    if (workStack != NULL)
      printf("       Shared Stack Push = %6ld    Shared Stack Pops = %6ld\n",
