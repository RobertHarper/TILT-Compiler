  ...
  if (inHeap(primary,fromSpace)) {
    ...
    *root = replica;
    if (!inHeap(replica,toSpace)) {
      printf("root = %lx   primary = %lx   replica = %lx\n", (long)root, (long)primary, (long)replica);
    }
...
void GCRelease_SemiConc(Proc_t *proc)
{
  ...
  if (shouldDoubleAllocate()) {
    if (collectDiag >= 3)
      printf("Proc %d:   Double-allocating %lx to %lx\n",proc->procid,(long)allocCurrent,(long)allocStop);
...
static void do_work(Proc_t *proc, int additionalWork)
{
  ...
  assert(proc->work.hasShared == 0);
  if (collectDiag >= 2)
    printf("Proc %d: Completed do_work.  %ld bytes allocated and %ld bytes copied on current GC cycle.\n", 
	   proc->procid, 
