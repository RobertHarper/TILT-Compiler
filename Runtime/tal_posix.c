...
ptr_t setRoundingMode(int ml_mode)
{
  ...
  switch (ml_mode) {
    ...
    case 2: mode = FE_UPWARD; break;
    case 3: mode = FE_DOWNWARD; break;
    default :
      runtime_error_fmt("setRoundingMode given unknown ML rounding mode %d", ml_mode);
    }
  fesetround(mode);
...
string posix_ascTime (mltm mltm)
{
  ...
  string res = alloc_string(strlen(result),result);
  return res;
}
...
string posix_strfTime(string_mltm arg)
{
  ...
  struct tm tm;
  strftime (buf, sizeof buf, string_get_elts(arg->a), mltm2ctm(arg->b, &tm));
...
{
  char buf[] = "TILT_File_XXXXXX";
  if (mkstemp(buf) == -1) {
    runtime_error_msg ("posix_os_tmpname failed");
  }
  string res = alloc_string(strlen(buf),buf);
...
{ 
  int bytes_read = read(fd, (string_get_elts(buf)) + start, len);
  if (bytes_read == -1) {
    runtime_error(errno);
  }
  return bytes_read;
...
{
  int written = write(fd, (string_get_elts(buf)) + start, len);
  if (written == -1) {
    runtime_error(errno);
  }
  return written;
...
static termio_rep ctermios2mltermios(struct termios* termios)
{
  ...
  return (termio_rep) alloc_record(fields, arraysize(fields));
}
...
static struct termios* mltermios2ctermios(struct termios* termios, termio_rep rep)
{
  ...
  memcpy(termios->c_cc, string_get_elts(rep->cc), NCCS);
  return termios;
}
...
static string_word_stringlist cgroup2ml(struct group* group)
{
  ...
  return (string_word_stringlist) alloc_record(fields, arraysize(fields));
}
...
static string_word_word_string_string cpasswd2ml(struct passwd* passwd)
{
  ...
  return (string_word_word_string_string) alloc_record(fields, arraysize(fields));
}
...
int_int_int_int_int posix_procenv_times(unit unused)
{
  ...
  return (int_int_int_int_int) alloc_record(fields, arraysize(fields));
}
...
string_option posix_procenv_getenv(string mlname)
{
  ...
  string mlvalue = (cvalue == NULL) ? 0 : alloc_string(strlen(cvalue),cvalue);
  return mlvalue;
}
