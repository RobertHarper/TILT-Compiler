...
Depending on whether \cd{'a} is \cd{unit} or not, the above
optimizations would give different representations for applications of
\cd{B}.  Therefore, if no special care were taken, then the
constructor \cd{B} would be forced to check, possibly at run-time if
not determinable earlier, whether \cd{'a} is \cd{unit}.  

In our scheme, we decide at compile-time that we will never specialize
\cd{B} whether \cd{'a} is instantiated at type \cd{unit} or not;
the result of applying \cd{B} will always be a pointer to a 
value of type \cd{'a}.
\smallskip
