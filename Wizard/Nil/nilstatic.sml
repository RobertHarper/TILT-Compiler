...
*)
  ...
  fun is_type' D kind = 
    (case K.expose (kind_standardize(D,kind))
       of Type_k => true
	| SingleType_k _ => true
	| _ => false)
  ...
  and kind_valid' (D : context, kind : kind) : kind = 
    (case K.expose kind of
       Type_k => kind
     | SingleType_k con => (con_analyze(D,con,K.Type_k);kind)
	 ...
	 in  K.Arrow_k(openness,formals,kind_valid (D,return))
	 end)
    ...
    in
	 ...
	 (Record_c (labels,SOME vars)) =>
	   ...
	   in if labels_distinct labels then ()
	      else (c_error(D,C.Prim_c(pcon,args),"DepRecord contains duplicate field labels"))
	   end
  ...
  and con_analyze (D : context, constructor : con,kind : kind) : unit = 
    ...
    in
      (case (C.expose constructor,K.expose kind) of
	 (_,SingleType_k c) => 
	 ...
	 (case Sequence.toList defs
	    of [(v,c)] => type_analyze (if is_recur then insert_stdkind (D,v,K.Type_k) else D) c
	     | _ => c_error(D,constructor,"Mu::Type has wrong number of arms"))
       | (Mu_c (is_recur,defs),Record_k lvk_seq) =>
	 let 
	   val D = if is_recur 
		     then let fun folder((v,_),D) = insert_stdkind (D,v,K.Type_k)
	 ...
	 in ()
	   ...
	   let
		 ...
		 val con = C.Let_c (Sequential,[bnd],C.Var_c var')
       ...
       | (Closure_c (code,env),Arrow_k(Closure,_,_)) => 
	 let
	   val code_kind =  con_valid (D,code)
	   (*Or could synthesize on the environment*)
	   val ((v,KE),vklist,body_kind) = 
	     case K.expose code_kind of
	       Arrow_k (Code ,c_parm::vklist,body_kind) => (c_parm,vklist,body_kind)
	     | _ => (c_error(D,constructor,"Invalid closure: code has wrong kind"))
		 
	   val kind2 = K.Arrow_k(Closure,vklist,body_kind)
	   val _ = sub_kind(D,kind2,kind)
	   val _ = (con_analyze(D,env,KE)) handle e => k_error(D,KE,"Illegal kind in Closure2?")
	 in ()
	 end
	 ...
	 in if sub_kind(D,body_kind,kind) then ()
	    else k_error(D,kind, "Analyzed kind and return kind from app don't match")
	 end
       | (Typecase_c {arg,arms,default,kind=given_kind},_) => 
	 let
	   ...
	   fun doarm (pcon,args,body) = 
		 ...
		 in
		   (C.Var_c v,insert_stdkind(D,v,k))
		 end
  ...
  and type_analyze(D,c) = con_analyze(D,c,K.Type_k)
	...
	(case C.expose constructor of
	 ...
	 | (Mu_c (is_recur,defs)) =>
	   let
	     val D =
	       if is_recur then
		 let fun folder((v,_),D) = insert_stdkind (D,v,K.Type_k)
	   ...
	   in  
	     if (Sequence.length defs) = 1
	       then K.SingleType_k(constructor)
	     else 
	       let 
		 fun mapper (i,(v,c)) = 
		   let val label = generate_tuple_label(i+1)
		   in((label,derived_var v),K.SingleType_k(C.Proj_c(constructor,label)))
		   end
	 ...
	 | (Typeof_c exp) => (K.SingleType_k(exp_valid (D,exp)))
	 | (Closure_c (code,env)) => 
	   let
	     val code_kind =  con_valid (D,code)
	     ...
	     val kind = K.Arrow_k(Closure,vklist,body_kind)
	 ...
	 | (Crecord_c entries) => 
	   ...
	   in K.Record_k (Sequence.fromList k_entries)
	   end
	 | (Proj_c (rvals,label)) => 
	   let
	     (*This will never be dependent*)
  ...
  and lambda_valid (D,Tag,formals,body) = 
    ...
    in K.Arrow_k(Tag,formals,body_kind)
    end		  

  (*PRE: kind1 and kind2 not necessarily normalized *)
  and sub_kind arg = subeq_kind false arg
  and kind_equiv (arg as (_,k1,k2)) = 
    let 
      val res = subeq_kind true arg
      val _ = (*if res then K.equate (k1,k2) else*) ()
    in res 
    end
  ...
  and subeq_kind is_eq (D,kind1,kind2) = 
	...
	foldl_all2 sub_one (D,Alpha.empty_context(),Alpha.empty_context()) (vks1,vks2)
      ...
      val res = 
      (case (K.expose kind1,K.expose kind2) 
...
*)
  ...
  and type_equiv (D,c1,c2,sk) = con_equiv(D,c1,c2,K.Type_k,sk)
  ...
  and con_equiv (args as (_,c1,c2,_,sk)) : bool = 
    let 
      val res =     flagtimer (equiv_total_profile,"Tchk:equiv_total", con_equiv_wrapper) args
      val _ = (*if res andalso not sk then C.equate (c1,c2) else*) ()
    in res 
    end
  ...
  and con_equiv_wrapper args = 
	  ...
	  val res = 
		...
		let 
		  ...
		  fun folder (((l,v),k),(state as (D,alpha),equal)) =
		    let 
		      ...
		      val equal = equal andalso con_equiv'(D,C.Proj_c(c1,l),C.Proj_c(c2,l),k,sk)
		      val state = bind_kind_eqn'(state,v,C.Proj_c(c1,l),k)  
		    in  (state,equal)
		    end
		in (case (C.expose c1,C.expose c2) 
		...
		let 
		  fun folder ((v,k),(D,alpha)) = 
			...
			in (C.Var_c vnew,(D,alpha))
			end
		      else (C.Var_c v,(insert_kind(D,v,k),alpha))
		    end
		  val (args,(D,rename)) = foldl_acc folder (D,Alpha.empty_context()) vklist
		  val k = alphaCRenameKind rename k
		  val c1 = C.App_c(c1,args)
		  val c2 = C.App_c(c2,args)
	...
	let 
	  fun folder((vopt1, c1), (vopt2, c2), (D, eRename, cRename, match)) = 
	    let 
	      ...
	      val match = match andalso con_equiv(D,c1,c2,K.Type_k,sk)
	...
	let val res =   
	      (case (C.expose c1,C.expose c2) of
		 ...
		 in
		   ...
		   eq_list(fn (c1,c2) => con_equiv(D,c1,c2,K.Type_k,sk'),
				   clist1,clist2)
		 end
		 ...
		 let 
		   fun do_mu () = 
			 ...
			 let
			   val (D,rename) = 
				 ...
				 let val vnew = Name.derived_var v1
				     val rename = Alpha.rename (rename,v1,vnew)
				     val rename = Alpha.rename (rename,v2,vnew)
				 in (insert_kind(D,vnew,K.Type_k),rename) end
			       else (insert_kind(D,v2,K.Type_k),Alpha.rename(rename,v1,v2))
			     else (insert_kind(D,v1,K.Type_k),
			 ...
			 in con_equiv(D,c1,c2, K.Type_k, false) (* no subtyping *) end
		     in  
...
(* Term level type checking.  *)
  ...
  and bnd_valid'' (bnd,state) = 
	...
	let
	  ...
	  val D = foldl (fn (v,D) => insert_con (D,v,C.Prim_c (Float_c F64,[]))) D fFormals
	in
	...
	let
	  ...
	  val closure_type = C.AllArrow_c {openness=Closure, effect=effect, 
	...
	(case bnd 
	   of Con_b (phase, cbnd) => 
	     let
	       val con = fn var => C.Let_c(Sequential,[cbnd],C.Var_c var)
	       val D = (case cbnd of 
  ...
  and exp_valid' (D : context,exp : exp) : con = 
	...
	(case value of
	   int (intsize,_) => C.Prim_c (Int_c intsize,[])
	 ...
	 | vector (con,vec) =>  
		 ...
		 let
		   ...
		   val _ = 
		     ...
		     (e_error(D,E.Const_e value,"Vector contains expression of incorrect type" handle e => raise e))
		 in()
		 end
	     in
	       (Array.app (fn e => exp_analyze(D,e,con)) vec;
		C.Prim_c (Vector_c,[con]))
	     end
	 | tag (atag,con) => (type_analyze (D,con);C.Prim_c (Exntag_c,[con]))
	 | array (con,arr) => e_error(D,E.Const_e value,"Array's shouldn't happen")
	 | refcell expref => e_error(D,E.Const_e value,"Ref's shouldn't happen"))
	...
	let
	  ...
	  in
	  fun do_default (D,NONE,[],result_type) = e_error(D,E.hide(Switch_e switch),"Case must be non-empty")
...
*)
	  fun sum_exn (mk_itype,check_arg) (D,arg,bound,arms,default,result_type) = 
		...
		(type_analyze(D,result_type);
		 ...
		 (e_error(D,E.hide (Switch_e switch),"Switch argument does not match found type"));
		 
		 if all do_arm arms then ()
		 else e_error(D,E.hide(Switch_e switch),"Arms to switch are invalid");
		 ...
		 let 
		   ...
		   val _ = 
		     (
		      ...
		      else e_error(D,E.hide (Switch_e switch), "Integer size mismatch in int switch");
	...
	let 
	  val orig_exp = E.Prim_e(NilPrimOp prim,cons,exps)
	  ...
	  fun inject_sum_nontag (check_con) (D,sumtype,sumcon,exps) = 
		...
		(case (Int.compare (which,0),Int.compare (nontagcount,1)) of
		 ...
		 | (EQUAL,EQUAL) => carrier
		 | _ => C.Proj_c(carrier,NilUtil.generate_tuple_label (which + 1)))
	  ...
	  val res = 
	    (case (prim,cons,exps) of
		 ...
		 in if (labels_distinct labels) then C.Prim_c (Record_c (labels,NONE),cons)
	    ...
	    | (make_vararg (openness,effect),[argc,resc],[e]) =>
	       let
		 val etype = C.AllArrow_c{openness=openness,effect=effect,isDependent=false,
					  tFormals=[],eFormals=[(NONE,argc)],fFormals=0w0,body_type=resc}
		 val _ = type_analyze(D,etype)
		 val _ = exp_analyze (D,e,etype)
	       in  C.Prim_c(Vararg_c(openness,effect),[argc,resc])
	       end
	    | (make_onearg (openness,effect),[argc,resc],[e]) =>  
	       let
		 val etype = C.Prim_c(Vararg_c(openness,effect),[argc,resc])
		 val _ = type_analyze(D,etype)
		 val _ = exp_analyze (D,e,etype)
	    ...
	    | (prim,cons,exps) => 
			(perr_e (E.Prim_e (NilPrimOp prim,cons,exps));
	...
	(case E.expose exp of
	 ...
	 | Handle_e {body,bound,handler,result_type} => 
	     let
	       val _ = exp_analyze(D,body,result_type)
	       val D = insert_con(D,bound,C.Prim_c(Exn_c,[]))
	...
	let
	  val con = exp_valid(D,E.Var_e var)
	in  ()
	end
	| export_valid D (ExportType (label,var)) = 
	let
	  val kind = con_valid(D,C.Var_c var)
	in ()
	end
...
structure WNilStatic = NilStatic
