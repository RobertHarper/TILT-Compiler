...
structure NilContextPre :> NILCONTEXTPRE = 
 struct
...
(*   val timer    = Stats.subtimer'
  ...
  fun subtimer (_,f) args = f args
    ...
    *)
   fun name_proj (SOME name,l) = SOME (C.Proj_c(name,l))
   ...
   fun name_app (SOME name,formals) = 
     ...
     in SOME (C.App_c(name,actuals))
     end
     | name_app (NONE,_)            = NONE
    
   (* Use the name in a Singleton if possible
    *)
   fun name_eqn (SOME name,_) = K.SingleType_k name
   ...
   fun insert_stdkind (context as {conmap,kindmap,counter}: context,var,std_kind) = 
     let
       val entry = {eqn = ref NONE,
		    ...
		    max_kind = delay (fn () => selfify (C.Var_c var,std_kind)),
		    index = counter}
     in
       {conmap = conmap, 
	counter = counter+1,
   ...
   and kind_standardize'(D : context, kind : kind, name : con option) : kind = 
	...
	(case K.expose kind of
	 ...
	 | Arrow_k (openness, formals, return) => 
	     ...
	     in K.Arrow_k(openness, formals, return)
	     end)
    in res 
    end
   ...
   and kind_of' (D : context,constructor : con,name : con option) : NilSubst.con_subst * kind = 
	 ...
	 (case C.expose constructor 
	    of Prim_c _        => (empty_subst(),name_eqn(name,K.SingleType_k(constructor)))
	     | (AllArrow_c _)  => (empty_subst(),name_eqn(name,K.SingleType_k(constructor)))
	     | ExternArrow_c _ => (empty_subst(),name_eqn(name,K.SingleType_k(constructor)))
	     | Typeof_c _      => (empty_subst(),name_eqn(name,K.SingleType_k(constructor)))
	      
	     | (Mu_c (recur,defs)) => 
	      let val len = Sequence.length defs
		  val kind = 		   
		    if len = 1 then name_eqn(name,K.SingleType_k(constructor))
		    else let 
			   fun mapper (i,(v,c)) = 
			     let val label = generate_tuple_label(i+1)
			         val kind = name_eqn(name_proj(name,label),K.SingleType_k(C.Proj_c(constructor,label)))
		...
		fun Pi(D,subst,Tag,formals,body) = 
		  let 
		    ...
		    val kind = K.Arrow_k(Tag,formals,return)
		  in
		    (subst,kind)
		  end		  
 		...
 		fun add_bnd (D,subst) maker (var,formals,body) = 
		  let
		    ...
		    val con = C.Let_c (sort,[bnd],C.Var_c var')
		    val subst = NilSubst.C.addr (subst,var,con)
		    val D = insert_kind(D,var,K.Single_k con)
		  in (D,subst)
		  end
		...
		fun loop ([],(D,subst)) = 
	     ...
	     | (Closure_c (code,env)) => 
	      ...
	      in (NilSubst.C.addl (TEv,env,subst),K.Arrow_k(Closure,vklist,body_kind))
	      end
		...
		val k_entries =  map mapper entries
	      in (empty_subst(),K.Record_k (Sequence.fromList k_entries))
	      end
	    ...
	    | (App_c (cfun,actuals)) => 
	      let val (subst,k) = kind_of' (D,cfun,NONE)
	      in
		(case K.expose k 
		   of Arrow_k (_,formals,body_kind) => 
		     let 
   ...
   and insert_kind (context as {conmap,kindmap,counter}:context,var,kind) = 
	 ...
	 val max_kind = delay (fn () => if delayed std_kind 
					  then kind_standardize' (context,kind,SOME (C.Var_c var))
					else selfify(C.Var_c var,thaw std_kind))
  ...
  fun insert_equation (context,var,con) = 
    insert_kind_equation(context,var,con,K.Single_k con)
  ...
  fun find_kind_equation (s as {kindmap,...}:context,con) : con option = 
	...
	(case K.expose k of
	   Record_k lvk_seq => 
		 ...
		 if (Name.eq_label(l,l'))
		   then (KIND (k,subst))
		 else loop (add subst (v,C.Proj_c(c,l'))) rest
	     in  loop subst lvk_list
	     end
	 | Single_k c => CON(substConInCon subst (C.Proj_c(c,l)))
	 | _ => 
	...
	(case K.expose k 
	   of Arrow_k (openness,vklist, k) => 
	    ...
	    | Single_k c => CON(C.App_c(substConInCon subst c,clist))
	    | _ => error (locate "app_kind") "bad kind to app_kind")

      fun closure_kind (k,env,subst) = 
	(case K.expose k
	   of Single_k c => CON(C.Closure_c(substConInCon subst c,env))
	    | Arrow_k (Code,(TEv,_)::vklist,body_kind) => 
	     KIND (K.Arrow_k(Closure,vklist,body_kind),add subst (TEv,env))
	...
	(case C.expose c 
	   of (Var_c v) => 
	    ...
	    | (Proj_c (c,l)) => 
	      (case traverse c of
		 CON c => CON(C.Proj_c(c,l))
	       | KIND (k,subst) => project_kind(k,c,l,subst)
	       | NON_PATH => NON_PATH)
	    | (App_c(c1,clist)) =>
	      (case traverse c1 of
		 CON c => CON(C.App_c(c,clist))
	       | KIND (k,subst) => 
		   app_kind(k,c1,clist,subst)
	       | NON_PATH => NON_PATH)
	    | (Closure_c(c1,c2)) => 
	      (case traverse c1
		 of CON c1 => CON(C.Closure_c(c1,c2))
