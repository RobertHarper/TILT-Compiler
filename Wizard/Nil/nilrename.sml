  ...
  struct
	...
	(case C.expose con
	   of Var_c var => 
		...
		of SOME var => (CHANGE_NORECURSE (state,C.hide(Var_c var)))
		 | _ => NORECURSE)
	    | _ => NOCHANGE)
	...
	(case E.expose exp
	   of Var_e var => 
		...
		of SOME var => (CHANGE_NORECURSE (state,E.hide(Var_e var)))
		 | _ => NORECURSE)
	    | _ => NOCHANGE)
	...
	if (Alpha.is_empty alpha_c) andalso (Alpha.is_empty alpha_e)  then NORECURSE
	  ...
	  (case C.expose con
	     of Var_c var => 
		  ...
		  CHANGE_NORECURSE(state,C.Var_c (Alpha.substitute(alpha_c,var)))
		else
		  NORECURSE)
	      | _ => NOCHANGE)
	...
	if (Alpha.is_empty alpha_e) andalso (Alpha.is_empty alpha_c)  then NORECURSE
	  ...
	  (case E.expose exp
	     of Var_e var => 
		  ...
		  CHANGE_NORECURSE(state,E.Var_e (Alpha.substitute(alpha_e,var)))
		else
		  NORECURSE)
	      | _ => NOCHANGE)
