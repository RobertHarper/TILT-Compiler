(*$import List Int TP_RECON GLOBAL INTSYN NAMES PATHS WHNF UNIFY ABSTRACT TYPECHECK STRICT PRINT TIMERS *)
  ...
  fun arrow (tm1, tm2) (G, SS) =
	...
	case SS L2
	  of ((IntSyn.Nil, L2'), _) =>
	   ...
	   | ((S, V'), _) =>
	      (error (r, "function type applied to argument");
	       (* ignore extraneous argument *)
	       ((IntSyn.Pi ((D1, IntSyn.No), IntSyn.EClo(V2,IntSyn.shift)), L2),
		Paths.bind (r, SOME(oc1), oc2)))
      end
  ...
  fun pi (dec, tm, r1) (G, SS) =
	...
	case SS L2
	  of ((IntSyn.Nil, L2'), _) =>
	   ...
	   (* can the next case arise? *)
	   | (S, V') =>
	       (error (r, "dependent function type applied to argument");
		((IntSyn.Pi ((D1, IntSyn.Maybe), V2), L2), (* L2 = L2' *)
		 Paths.bind (r, oc1Opt, oc2)))
	...
	case S
	  of IntSyn.Nil => ((IntSyn.Uni (IntSyn.Type), V), Paths.leaf r)
	   ...
	   | _ => (error (r, "`type' applied to argument");
		   ((IntSyn.Uni (IntSyn.Type), V), Paths.leaf r))
      end
