...
fun print_term (Var v) = (print_string "v"; print_int v)
  | print_term (Prop (head,argl)) = (print_string "(";
				     print_string (#name head);
				     app (fn t => (print_string " "; print_term t)) argl;
				     print_string ")")
			 

fun myprint_term term = () (* print_term term *)
...
fun get_binding v list =
  let val _ = myprint "get_binding entered\n"
      val rec get_rec = 
	  fn [] => (myprint "get_binding raising unbound\n";
		    failwith "unbound")
	   | Bind(w,t)::rest => if v = w  then t else get_rec rest
      val res = get_rec list 
      val _ = myprint "get_binding returning\n"
  in  res
  end

fun apply_subst alist term =
    let fun as_rec (Var v) = ((get_binding v alist) handle Failure _ => term)
	  | as_rec (Prop (head,argl)) = Prop (head, map as_rec argl)
	val _ = (myprint "apply_subst called on "; myprint_term term; myprint "\n")
	val res = as_rec term 
    in  res
    end
...
and unify1 term1 term2 unify_subst =
    let val _ = (myprint "    unify1 called with: "; myprint_term term1;
		 myprint "    \n             and: "; myprint_term term2;
		 myprint "\n")
	val res = case term2 of
	    Var v =>
	...
	val _ = myprint "    unify1 returning\n"
    in res
    end
...
fun rewrite term = 
    let val _ = (myprint "  rewrite called with: "; myprint_term term; myprint "\n")
	val res = case term of
	    Var _ => term
	  | Prop (head, argl) =>
...
and rewrite_with_lemmas term lemmas =
    let val _ = (myprint "  rewrite_with_lemmas called with: "; myprint_term term; myprint "\n")
	val res = case lemmas of
	    [] => term
	  | (t1,t2)::rest =>
		((rewrite (apply_subst (unify term t1) t2))
		 handle Unify => (myprint "  rewrite_with_lammas caught Unify exn\n";
				  rewrite_with_lemmas term rest))
	val _ = (myprint "  rewrite_with_lemmas finished: "; myprint_term term; myprint "\n")
    in  res
    end
...
((* print_term x; print_string"\n"; *)
  let val _ = myprint "tautp: calling rewrite\n"
      val y = rewrite x 
      val _ = (myprint "tautp: computed y\n"; myprint_term y; myprint "\n")
      val res = tautologyp y [] []
  in res
  end)
...
fun print_head ({name,props}:head) = 
    (print name; 
     map (fn (x,y) => (print_term x; print " = "; print_term y; print "\n")) 
     (!props); 
     print "\n\n")
