...
val _ = print "      union: checked b\n"
  ...
  fun AdvanceTok () : unit =
    let 
      fun isletter(x:char) = char_geq(x,#"a") andalso char_leq(x,#"z") 
	orelse char_geq(x,#"A") 
    ...
    in 
      (case !LexState of 0 => let val makeTok = fn () =>
	 case skipws() of
	   (* Lex % operators *)
	   #"%" => (case nextch() of 
		      #"%" => LEXMARK
    ...
    fun parse() : (string * (int list * exp) list * ((string,string) dictionary)) =
      let 
	val Accept = ref (create string_leq) : (string,string) dictionary ref
    ...
    fun readLexgenInput() =     (* Example input file is Bench/ml.lex *)
      let 
	val _ = (print "Enter filename: "; TextIO.flushOut TextIO.stdOut)
    ...
    fun runSomeLexgen str = lexGen str
      
    fun run () = 
      let in
	runSomeLexgen "../BenchData/ml.lex"(*;
	runSomeLexgen "../BenchData/ml1.lex";
	runSomeLexgen "../BenchData/ml2.lex";
	runSomeLexgen "../BenchData/ml3.lex"*)
      end
