...
*)
      val Indent  = Stats.int("FormatterIndent",3)
      and Skip    = ref 1
      and Blanks  = ref 1
...
*)
      local
         fun Repeat s acc 0 = acc
	   | Repeat s acc n = Repeat s (acc^s) (n-1)
...
fun Width0(m,   b,i, Str(n,_)) = (n,n)
 ...
 |  Width0(m,b,i, Deep f) = Width0(m,b,i,f)
...
\end{itemize}
*)
fun pphv(dep,mw,li,bl,is,ss,mp,ch,lb,nil,res)= (Max(mp,ch),res)
 |  pphv(dep,mw,li,bl,is,ss,mp,ch,lb,((gpwdth,flist,brk)::t),res) =
    let val (ch1,s1,mp) =
        (* horizontal width, string to print, max print width *)
            ...
            then (* OK - group fits within page or has to fit:
                    horizontal break *)
            let val (n,s)=print'p(dep,mw,li,bl,is,ss,Hori,lb,res)
            in (ch+n,s,mp) end
            else (* group will not fit: vertical break.
                    Was last line of maximum width? *)
            let val (n,s)=print'p(dep,mw,li,bl,is,ss,Vert,lb,res) in
                (n,s,Max(mp,ch))
            end
 (* Now print the elements of the group using default for horizontal tabs *)
       val (n2,s2) = pph(dep,mw,(li+ch1),bl,is,ss,flist,0,s1)
       (* Now print rest of horizontal-vertical box *)
    in pphv(dep,mw,li,bl,is,ss,mp,(ch1+n2),brk,t,s2) end
(*
%{\bf Improvements:}
...
\end{itemize}
*)
and ppv(dep,mw,li,ci,bl,is,ss,max,gw,nil,res) =
                          (Max(max,gw),res)
  | ppv(dep,mw,li,ci,bl,is,ss,max,gw, Dbk::t,res) =
        let val (n,s)   = print'p(dep,mw,li,bl,is,ss,Vert,Dbk,res)
        in ppv(dep,mw,li,(li+n),bl,is,ss,Max(max,gw), n,t, s) end
  |  ppv(dep,mw,li,ci,bl,is,ss,max,gw,(b as (Brk(_,_)))::t,res) =
        let val (n,s)   = print'p(dep,mw,li,bl,is,ss,Vert,b,res)
        in ppv(dep,mw,li,(li+n),bl,is,ss,Max(max,gw), n,t,s) end
  |  ppv(dep,mw,li,ci,bl,is,ss,max,gw,h::t,res) =
          let val (n,s)   = print'p(dep,mw,ci,bl,is,ss,Vert,h,res)
          in ppv(dep,mw,li,(ci+n),bl,is,ss,max,(gw+n),t,s) end
...
\end{itemize}
*)
and pph(dep,mw,id,bl,is,ss,nil,nres,sres) = (nres,sres)
  | pph(dep,mw,id,bl,is,ss, h::t,nres,sres) =
          let val (n,s)   = print'p(dep,mw,id,bl,is,ss,Hori,h,sres)
          in pph(dep,mw,(id+n),bl,is,ss,t,n+nres,s) end
(*
...
*)
and print'p(dep,mw,id,bl,is,ss,mo,  Str(n,s),res) = (n,s::res)
 |  print'p(dep,mw,id,bl,is,ss,Hori,Brk(b,i),res) =
           (b, (if (!Bailout) then Spmod(dep,b) else Sp(b))::res)
 |  print'p(dep,mw,id,bl,is,ss,Vert, Brk(b,i), res) =
           (i, (if (!Bailout) then Spmod'(dep,id+i) else Sp(id+i))::(Nl(ss))::res)
 |  print'p(dep,mw,id,bl,is,ss,Hori, Dbk, res) =
           (bl, (if (!Bailout) then Spmod(dep,bl) else Sp(bl))::res)
 |  print'p(dep,mw,id,bl,is,ss,Vert, Dbk, res)      =
           (is,(if (!Bailout) then Spmod'(dep,id+is) else Sp(id+is))::(Nl(ss))::res)
 |  print'p(dep,mw,id,bl,is,ss,mo,   Ebk,res)      = (0,res)
 |  print'p(dep,mw,id,bl,is,ss,mo,   Hbx((min,max),blanks,l),res) =
            if (!Bailout) andalso (id+min) >= mw
               andalso (id mod (!Pagewidth) >= !BailoutSpot)
            then pph(dep,mw+(!Pagewidth),mw + (!BailoutIndent),
                        blanks,is,ss,l,0,(Spmod'(dep,mw))::(Nl(ss))::res)
            else pph(dep,mw,id,blanks,is,ss,l,0,res)
 |  print'p(dep,mw,id,bl,is,ss,mo,   Vbx((min,max),indent,skip,l), res) =
           if (!Bailout) andalso (id+min) >= mw
               andalso (id mod (!Pagewidth) >= !BailoutSpot)
           then let val id = mw+(!BailoutIndent)
                in ppv(dep,mw+(!Pagewidth),id,id,bl,indent,skip,0,0,l,(Spmod'(dep,mw))::(Nl(ss))::res) end
           else ppv(dep,mw,id,id,bl,indent,skip,0,0,l,res)
 |  print'p(dep,mw,id,bl,is,ss,mo,   Hvx(((min,max),(nmode,xmode)),blanks,indent,skip,l), res) =
            let val gl=gh(nil,l,nil) in
            if (!Bailout) andalso (id+min) >= mw
               andalso (id mod (!Pagewidth) >= !BailoutSpot)
            then pphv(dep,mw+(!Pagewidth),mw+(!BailoutIndent),
                          blanks,indent,skip,0,0,Ebk,gl,(Spmod'(dep,mw))::(Nl(ss))::res)
            else pphv(dep,mw,id,blanks,indent,skip,0,0,Ebk,gl,res)
            end
 |  print'p(dep,mw,id,bl,is,ss,mo,   Hov(((min,max),(nmode,xmode)),blanks,indent,skip,l), res) =
             if (max<=(mw-id))
             then if xmode=Hori
                     then pph(dep,mw,id,blanks,is,ss,l,0,res)
                  ...
                  then if nmode=Hori
                       then pph(dep,mw+(!Pagewidth),mw + (!BailoutIndent),
                            ...
                            in ppv(dep,mw+(!Pagewidth),id,id,
                                  blanks,indent,skip,0,0,l,(Spmod'(dep,mw))::(Nl(ss))::res) end
                  else if nmode=Hori
                       then pph(dep,mw,id,blanks,is,ss,l,0,res)
                       else ppv(dep,mw,id,id,blanks,indent,skip,0,0,l,res)
...
*)
      ...
      fun debug_output_fmt(Formatstream fs, fm) =
                  let val mw = (!Pagewidth)
                            ...
                            val (w,s) = print'p(0,!Pagewidth,0,!Blanks,!Indent,!Skip,Hori,fm, nil)
                          in
                             output(fs,
