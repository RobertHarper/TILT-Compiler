	...
	*)
	...
	local
		...
		fun try_exn_printers e =
			let
				fun loop printers : string option =
					(case printers of
						nil => NONE
					|	pr::prs =>
							let	val sopt = pr e
							in	if isSome sopt then sopt
								else loop prs
							end)
			in	loop(!exn_printers)
			end
	in
		fun register_exn_printer (pr:exn -> string option) : unit =
			exn_printers := pr :: (!exn_printers)
