(*
	The runtime needs to know about these exceptions.  we register
	them with the runtime by calling these c functions.  Note that
	all that matters is the stamp: the values passed with SysErr
	and LibFail are ignored.
*)
