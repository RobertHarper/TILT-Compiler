         ...
         *)
          ...
          fun toString (BA{nbits=0,...}) = ""
            | toString (BA{nbits,bits}) = let
                ...
                fun put (i : word, j : word) = let
                      val v = bits sub (P.uint32toint32 i)
                      in
                        W8A.update (buf, P.uint32toint32 j, W8V.sub(hexs, Word8.toInt(v & #"\015")));
                        W8A.update (buf, P.uint32toint32(j-0w1), W8V.sub(hexs, Word8.toInt(v >> 0w4)));
                        put (i+0w1, j-0w2)
                      end
                in
                  (put(0w0,P.int32touint32(len-1))) handle _ => ();
          ...
          fun fromList [] = array (0, false)
            | fromList l = let
                ...
                fun getbyte ([],_,b) = ([],b)
                  | getbyte (l,#"\000",b) = (l,b)
                  | getbyte (false::r,bit : char, b) = getbyte(r,bit << 0w1,b)
...
structure BitVector :> BIT_VECTOR = BitArrayAll.vector
