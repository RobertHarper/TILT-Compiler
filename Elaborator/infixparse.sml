  ...
  struct
    ...
    fun driver(objlist       : 'a list,
	  ...
	  fun get_fix obj = (case (get_fixity obj) of
					NONE => (Error.error_region();
						 print "obj has no fixity: ";
						 print_obj obj;
						 print "\n";
						 raise Stop)
				      | SOME f => f)
	...
	fun self_one arg = (case (parse_pat(table,is_nonconst_constr,[arg])) of
				[p] => p
			      | pats => (Error.error_region();
					 print "parse_pat on subcall yielded multiple patterns\n";
	...
	let
	    ...
	    fun loop (a::rest) seen = if (Listops.member_eq(Symbol.eq, a, seen))
					  then (Error.error_region();
						print "datbind/withbind binding duplicate variables";
						raise Stop)
