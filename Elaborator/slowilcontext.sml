functor SlowIlContext(structure Il : ILLEAK) 
    : ILCONTEXT = 
    struct
    ...
    fun context_entries (CONTEXT {flatlist,...}) = flatlist
    fun change_flatlist (CONTEXT{flatlist, fixity_list, label_list, var_list, tag_list}, flat) = 
	CONTEXT{flatlist = flat @ flatlist,
		fixity_list = fixity_list,
		label_list = label_list,
		var_list = var_list,
		tag_list = tag_list}
    fun add_context_sdecs(ctxt, sdecs) = change_flatlist(ctxt, map CONTEXT_SDEC sdecs)
    fun add_context_sdec(ctxt, sdec) = add_context_sdecs(ctxt,[sdec])
    ...
    fun add_context_inline(ctxt, l, v, inline) = change_flatlist(ctxt,
								 [CONTEXT_INLINE(l,v,inline)])
    fun add_context_entries(ctxt, e : context_entry list) : context = 
	change_flatlist(ctxt, e)
    ...
    fun Context_Get_FixityTable (CONTEXT{flatlist,...}) : fixity_table = 
	let 
	    fun help (CONTEXT_FIXITY vflist) = vflist
	      | help _ = []
	    val res = List.concat (map help flatlist)
	in  res 
	end
     ...
     (* ---------- Bound Rules from page 15 - 16 ----------------- *)
		...
		fun Context_Bound (CONTEXT {flatlist,...}) = List.mapPartial Ctxt_Bound flatlist
	    end
		...
		| (CONTEXT_SDEC(SDEC(l,dec))::r) =>
	      ...
	      | [] => ((* debugdo (fn () => (print "Context_Lookup failed on ";
			    ...
			    raise (NOTFOUND ("Context_Lookup failed on " 
					     ^ (label2string lab)))))
	  val CONTEXT {flatlist = context_entries,...} = context
	in case labels of
 ...
 pp_list pp_label' labels ("","",".",false); *)
	  ...
	  (#2(Context_Lookup arg)
	       handle e => ((* print "error in Context_Lookup with labels = ";
			    ...
			    print "\n"; *)
			    raise e))
      fun add_context_fixity (ctxt,fixity) = change_flatlist(ctxt, [CONTEXT_FIXITY fixity])
      fun var2label (CONTEXT {flatlist=entries,...}, tarv) = 
      ...
      fun Context_Exn_Lookup (CONTEXT {flatlist=entries,...}, tag) = 
