		  ...
		  | Fixopen_b vfset =>
			...
			in
			    (#bnd f) (Fixopen_b 
				      (Util.list2set 
				       (map (fn (v,fc) =>
					     (v, xfunction f fc))  vflist)))
			end
		  | _ => raise UNIMP
	    end    
	and xfunction (f as { bnd, exp, func, switch}) 
	    (Function(eff, r, vks, vcs, vs, expr, con)) = 
	...
	and xswitch  (f as { bnd, exp, func, switch}) s =
		...
		fun do_sw  do_t {arms,default,arg,info } =
			...
			val newdefault = case default of 
			  ...
			  | NONE => NONE 
		    in {arms = newarms, default = newdefault, 
			arg = arg, info= info}
		    end
	    in
		case s of
	...
	and xexp (f as { bnd, exp, func, switch})  expr =
		...
		case expr of 
		  ...
		  | App_e (openness, func, cons, exps1, exps2) =>
			#exp f (App_e (openness, self func, cons, 
				       map self exps1,map self exps2))
